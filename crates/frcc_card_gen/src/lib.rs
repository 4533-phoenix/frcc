...
pub fn test() {
    ...
    resvg::render(
        &Tree::from_str(include_str!("../../../cards/back/default.svg"), &options).unwrap(),
        Transform::identity(),
        &mut pixmap.as_mut(),
    );
    resvg::render(
        &Tree::from_str(
            &gen_fiducial("FRCC-12345678910".to_owned()),
            &Options::default(),
        )
        .unwrap(),
        Transform::identity().pre_translate((630.0 / 2.0) - 100.0, (880.0 / 2.0) - 100.0),
        &mut pixmap.as_mut(),
    );
